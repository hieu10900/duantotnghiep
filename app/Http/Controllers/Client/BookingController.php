...
//        end
        ...
        $request['price_room'] = $priceOfRoom->price * ($check_out->diff($check_in)->format("%a"));
        ...
        if (!empty($request['services'])) {
            ...
            " .  $ses->name . " Giá : " . $ses->price . " VND * " . $request['amount_of_people'] . " = " . $request['amount_of_people'] * $ses->price;
    ...
    public function detail(BookingRequesr $bookingRequesr)
    {
        ...
        $priceOfRoom = Room::find($request['room_id']);
        $priceOfBooking = $priceOfRoom->price * ($check_out->diff($check_in)->format("%a"));
        $room = "Phòng :" . $priceOfRoom->name . "  Giá :" . number_format((float)$priceOfRoom->price) . " * "
            . ($check_out->diff($check_in)->format("%a") )
            . " đêm = " . number_format((float)$priceOfBooking) . " VND.
            ";
        ...
        if (empty($request['services']) == false) {
            ...
            foreach ($se as $ses) {
                $serviceOfBooking =  $serviceOfBooking . "
            " .  $ses->name . " Giá : " . number_format((float)$ses->price) . " VND * " . $request['amount_of_people'] . " = " . number_format((float)$request['amount_of_people'] * $ses->price) ." VND";
